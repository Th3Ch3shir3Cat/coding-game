...
public class Picture extends JPanel implements ActionListener {
    ...
    public Picture(int nroFichas, MainFrame nucleo) {
    ...
    private void inicializarComponentes() {
        ...
        for (int i = 1; i <= LIMITE_FICHAS; i++) {
            int w = nroFichas - i + 1;
            ImageIcon ii = new ImageIcon(this.getClass().getResource("/res/images/" + i + ".png"));
    ...
    private void inicializarComponentesDeAnimacion() {
        ...
        topes[3] = 0;
        ficha = 1;
        movimientos = new Moves[(int) Math.pow(2, nroFichas)];
        algoritmoHanoi(nroFichas, 1, 2, 3); // llena el vector de movimientos
        posiciones = new Position[9];
        for (int i = 1; i <= nroFichas; i++) {
            int w = nroFichas - i + 1;
            posiciones[i] = new Position(posicionXFicha(i, 1), posicionYFicha(w));
        }
        x = posiciones[1].getX();
        y = posiciones[1].getY();
        movimientoActual = 1;
    ...
    public void algoritmoHanoi(int n, int origen, int temporal, int destino) {
        ...
        nm++;
        movimientos[nm] = new Moves(n, origen, destino);
